...
public class MySphereMeshGenerator : MonoBehaviour {
    ...
    void AssignVertices(Node[] points)
    {
        for (int i = 0; i < points.Length; i++)
        {
            if (points[i].vertexIndex == -1)
            {
                ...
                vertices.Add(Spherize(points[i].position));
            }
        }
    }
    ...
    private Vector3 Spherize(Vector3 point)
    {
        float longitude = 360 * (point.x / (squareGrid.mapWidth - 1));
        float latRatio = point.z / squareGrid.mapHeight;
        //float latitude = 60 * Mathf.Sqrt(2*Mathf.Abs(latRatio)) * Mathf.Sign(latRatio);
        float latitude = Mathf.Rad2Deg * Mathf.Atan(Mathf.PI * latRatio);

        Vector3 newPoint = new Vector3(0, 10, 0);
        newPoint = Quaternion.AngleAxis(-longitude, Vector3.forward) * Quaternion.AngleAxis(latitude, Vector3.right) * newPoint;
