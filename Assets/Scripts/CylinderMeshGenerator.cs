...
public class CylinderMeshGenerator : MonoBehaviour
{
    ...
    void AssignVertices(Node[] points)
    {
        for (int i = 0; i < points.Length; i++)
        {
            if (points[i].vertexIndex == -1)
            {
                ...
                vertices.Add(Cylindrize(points[i].position));
            }
        }
    }
    ...
    private Vector3 Cylindrize(Vector3 point)
    {
        float longitude = 360 * (point.x / squareGrid.mapWidth);
        float latitude = point.z;

        Vector3 newPoint = new Vector3(0, 10, latitude);
        newPoint = Quaternion.AngleAxis(-longitude, Vector3.forward) * newPoint;
